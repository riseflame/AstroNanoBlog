...
Subagent是让模型自己创建一个新的会话，在会话中为Subagent去自动安排角色和任务，通过这种形式，可以提高模型的上下文的利用效率，Subagent和主Agent不共享上下文信息，工作处于串行状态，主Agent在subagent运行时会暂时挂起，直到subagent返回结果。调用该功能只需要在提示词里写好调用几个subagent去做xxxxx。常用的例子就是去讨论实现，和review代码，如下所示：

![image.png](https://img.zzliu.com/file/1773735381119_image.png)


Subaents还可以创建工作流，例如我有一系列复杂的任务需要完成，这些任务需要的上下文上限超出了一个agent的最大上下文窗口，此时我可以建立一个coordinator的agent，由他负责进行调度，创建其他subagent来完成任务，for example：
``` markdown
---
...
```
name字段规定了该agent的名字，agents字段规定了该模型可以调用的其他的agent名称，tools字段规定了该模型可以调用的工具明成，包括build-in工具，和mcp工具。models字段规定了该agent可以调用的模型，可以写一个也可以写一个list，当写一个list时，会从第一个开始尝试，直到成功为止，使用subagent时需要配置至少一个tools权限较高的模型，不然只能调用explore的agent。
更多具体的字段名称可以看[官方文档](https://code.visualstudio.com/docs/copilot/customization/custom-agents)。
## 2.2 MCP工具配置
